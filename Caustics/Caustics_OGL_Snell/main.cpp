...
for (int i=0;i<octaves;i++)
	{
	y -= factor*VTXSIZE*d*cos(timer*SPEED + 1.0f / factor*x*z*WAVESIZE);
//			(factor)*VTXSIZE*d*cos(((float)timer*SPEED)+(1/factor)*(x*z)*WAVESIZE) 
			;
...
// here's the main stuff for the caustic
{
	...
	point q(xi + QUADSIZE, 0, zi);
	q.y = func(xi + 1, zi);

	point r(xi, 0, zi + QUADSIZE); 
	r.y = func(xi, zi + 1);

	point e1 = q - p; 
	point e2 = r - p;	
	point n = e1^e2;	// the normal above the sampling point 
...
//光线折射方程式
	...
	static   float    s_maxu = 0.0f;
	static   float    s_maxv = 0.0f;
	static   float    s_minu = 1.0f;
	static   float    s_minv = 1.0f;
	float   _fixu = u / numrays;
	float   _fixv = v / numrays;
	if (s_maxu < _fixu)
		s_maxu = _fixu;
	if (s_maxv < _fixv)
		s_maxv = _fixv;
	...
	if (s_minv > _fixv)
		s_minv = _fixv;
//注意前面的系数的选择所代表的内涵,因为u,v被平均之后,其值的范围在-0.5*sqrt(2),0.5*sqrt(2)之间
//并且在实际的实践中,uv/numrays的值的范围通常在-0.4--0.3之间
	u = 4 * (u / numrays) + 0.5;
	v = 4 * (v / numrays) + 0.5;
}
...
for (int xi=0;xi<XFIELD;xi++)
	...
	for (int zi=0;zi<ZFIELD;zi++)
		...
		float u,v;
		sample_caustic(xi*QUADSIZE,zi*QUADSIZE,u,v);
		
		glTexCoord2f(u,v);
		glVertex3f(xi*QUADSIZE,-10,zi*QUADSIZE);

		// compute caustic environment mapping for point 2 in the strip (shift 1 in xi)
		sample_caustic((xi+1)*QUADSIZE,zi*QUADSIZE,u,v);
		
		glTexCoord2f(u,v);
		glVertex3f((xi+1)*QUADSIZE,-10,zi*QUADSIZE);
...
float a=1.0/cos(45.0*3.1416/180);
g_hInst=hInstance;
