...
HRESULT CompileShaderFromFile( WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
    ...
    memset(pFileData, 0, FileSize.LowPart);
    if( !pFileData )
...
void ocean_simulator::init_dx11(int hmap_dim, int output_size, UINT float2_stride)
{
    ...
    CompileShaderFromFile((WCHAR *)filePath.c_str(), "QuadVS", "vs_4_0", &pBlobQuadVS);
    CompileShaderFromFile((WCHAR *)filePath.c_str(), "UpdateDisplacementPS", "ps_4_0", &pBlobUpdateDisplacementPS);
    CompileShaderFromFile((WCHAR *)filePath.c_str(), "GenGradientFoldingPS", "ps_4_0", &pBlobGenGradientFoldingPS);
    assert(pBlobQuadVS);
