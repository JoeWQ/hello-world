...
//			  1、单STREAM，先用物体SetVertexShader(VS.Handle, VS.Decl)，然后VS.SetTransform（必须设置！！哪怕是NULL），最后用物体.DrawPrimitive即可(自动设置物体的VertexShader和Declaration)，用完要物体.RestoreVertexShader
//            2、多STREAM，用物体SetStreamSource设置流号，然后VS.SetTransform（必须设置！！哪怕是NULL），最后直接用VS.DrawPrimitive即可(自动设置VS.VertexShader)
...
private:
	LPD3DXBUFFER ComplicationErrors, CompiledShader;
