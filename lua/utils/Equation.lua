...
function Equation.creat_1_1_b(startP,endP,haveArea)
	...
	eqObj.inArea =function (self,p )
		...
		if(self.haveArea) then
			if p.x <= startP.x and p.x >= endP.x then
				--todo
				inX = true
			elseif p.x >=startP.x and p.x <=endP.x then
					--todo
					inX =true
			end
			local inY =false
			if p.y <=startP.x and p.y >=endP.y then
				--todo
				inY = true
			elseif p.y>=startP.y and p.y <= endP.y then
				--todo
				inY =true
			end
			if inX and inY then
...
function Equation.getSpeedBySEGH(s,e,g,h)
	...
	xSpeed=dx/t
	ySpeed=dy/t
	--print("speed2222:",xSpeed,ySpeed,zSpeed,dx,dy,dz,halfT,t)
