...
char *CubeMapNames[6] =
{
	"skypictures/ixpos.bmp",
	"skypictures/ixneg.bmp",
	"skypictures/iypos.bmp",
	"skypictures/iyneg.bmp",
	"skypictures/izpos.bmp",
	"skypictures/izneg.bmp"
};
...
{
	"skypictures/xpos.bmp",
	"skypictures/xneg.bmp",
	"skypictures/ypos.bmp",
	"skypictures/yneg.bmp",
	"skypictures/zpos.bmp",
	"skypictures/zneg.bmp"
};
...
Alaska::Alaska()
{
	...
	alpha_color_array[0] = 215;
	alpha_color_array[1] = 215;
	alpha_color_array[2] = 115;
...
//	scale_height = 1.0;//scale the wave heights
	deep = 120;
...
void Alaska::Cg_init()
{
	...
	v=cgCreateProgramFromFile(context,CG_SOURCE,"v.cg",vprofile,"vert",0);
	cgGLLoadProgram(v);

	fprofile=cgGLGetLatestProfile(CG_GL_FRAGMENT);
	cgGLSetOptimalOptions(fprofile);
	f=cgCreateProgramFromFile(context,CG_SOURCE,"f.cg",fprofile,"frag",0);
	cgGLLoadProgram(f);

	//////////////////////////////////////////////////////////////

	position=cgGetNamedParameter(v,"position");
	texCoord=cgGetNamedParameter(v,"texCoord");//new
	normal=cgGetNamedParameter(v,"normal");

	fresnelBias=cgGetNamedParameter(v,"fresnelBias");
	fresnelScale=cgGetNamedParameter(v,"fresnelScale");
	fresnelPower=cgGetNamedParameter(v,"fresnelPower");
	etaRatio=cgGetNamedParameter(v,"etaRatio");
	eyepositionW=cgGetNamedParameter(v,"eyePositionW");
	lightposition=cgGetNamedParameter(v,"lightPosition");//new
//	texrotmat=cgGetNamedParameter(v,"texrotmat");
	modelViewProj=cgGetNamedParameter(v,"ModelViewProj");
	modelToWorld=cgGetNamedParameter(v,"modelToWorld");

//	color=cgGetNamedParameter(f,"color");

	normalMapTexCoord=cgGetNamedParameter(f,"normalMapTexCoord");//new
	normalMap=cgGetNamedParameter(f,"normalMap");//new
	normalizeCube=cgGetNamedParameter(f,"normalizeCube");//new
	normalizeCube2=cgGetNamedParameter(f,"normalizeCube2");//new

//	reflectivity=cgGetNamedParameter(f,"reflectivity");
//	foam=cgGetNamedParameter(f,"foam");
	environmentMap=cgGetNamedParameter(f,"environmentMap");
	bottomMap=cgGetNamedParameter(f,"bottomMap");

	cgGLSetTextureParameter(normalMap,tex[3]);
//	cgGLSetTextureParameter(foam,tex[2]);
	cgGLSetTextureParameter(environmentMap,CubeMap);
	cgGLSetTextureParameter(bottomMap,BottomMap);
	cgGLSetTextureParameter(normalizeCube,cubetexture);//new
	cgGLSetTextureParameter(normalizeCube2,cubetexture);//new

}
///////////////////////////end Cg initialization///////////////////////

void Alaska::read_land_in()
{
	FILE *in;

	int i,j;
	float hold_doubles;
	if ((in=fopen("theland","r")) == (FILE*) NULL)
	{
		printf("couldn't open file %50s\n","theland");
		exit(0);
	}
	fscanf(in,"%i",&landusize);//can't believe I actually forgot the & here after all this time
	fscanf(in,"%i",&landvsize);

	// Allocate memory
	m_land = new double[landusize * landvsize * 3];
	m_land_normals = new double[landusize * landvsize * 3];

	landusize -=1;
	landvsize -=1;
	for (i=0;i<landusize;i++)
	{
		for (j=0;j<landvsize;j++)
		{
			fscanf(in,"%g",&hold_doubles);
			land(i, j, 0) = ((double) hold_doubles);
			fscanf(in,"%g",&hold_doubles);
			land(i, j, 1) = ((double) hold_doubles);
			fscanf(in,"%g",&hold_doubles);
			land(i, j, 2) = ((double)((int)hold_doubles));

			fscanf(in,"%g",&hold_doubles);
			land_normals(i, j, 0) = (double) hold_doubles;
			fscanf(in,"%g",&hold_doubles);
			land_normals(i, j, 1) = (double) hold_doubles;
			fscanf(in,"%g",&hold_doubles);
			land_normals(i, j, 2) = (double) hold_doubles;
		}
	}
	fclose(in);
}
...
GLubyte * Alaska::read_JPEG_file(char * filename)
{
	int i;
	int row_stride;
...
//	int row_stride;					//physical row width in output buffer
	...
	row_stride=cinfo.output_width * cinfo.output_components; //their row width
	row_width=cinfo.output_width * cinfo.output_components; //my row width
	col_height=cinfo.output_height; //this col height is not mult by three
	buffer=(*cinfo.mem->alloc_sarray)
				((j_common_ptr) &cinfo, JPOOL_IMAGE, row_stride,1);
	// prepare my array here
	byteptr=(GLubyte*)malloc(cinfo.output_width*
							 cinfo.output_height*cinfo.output_components);
	while (cinfo.output_scanline < cinfo.output_height)
	{
		(void) jpeg_read_scanlines(&cinfo,buffer,1); // get a line from jpg

		for (i=0;i<row_width;i++)
		{
		// the line below corrects the upside down way jpeg is read line by line
		*(byteptr+(cinfo.output_height-cinfo.output_scanline)
					*cinfo.output_components*cinfo.output_width+i)=(GLubyte)*(buffer[0]+i);
		}
	}
...
/////////////////////// end of jpeg ////////////////////////////////
GLuint Alaska::Loadjpg(char * filename)
{
	GLubyte * bptr;
	GLuint texture;
	bptr=read_JPEG_file( filename);
	...
	// ALWAYS invoke this line below if any change is made in the texture
	//OTHERWISE the change will not appear in the resulting image
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, row_width/3, col_height,
				0, GL_RGB, GL_UNSIGNED_BYTE, bptr);

	return texture;
}

GLuint Alaska::LoadDecalClampjpg(GLubyte * bptr,char * filename)
{
	GLuint texture;
	bptr=read_JPEG_file( filename);
	glEnable(GL_TEXTURE_2D);
...
GLuint Alaska::LoadRepeatjpg(GLubyte * bptr,char * filename)
{
	GLuint texture;
	bptr=read_JPEG_file( filename);
	glEnable(GL_TEXTURE_2D);
...
GLuint Alaska::LoadTexture2D(const char *const filename)//reads BMP files
{
...
///////////////////// function to read cube map /////////////////////
//This function creates a cube map, from global data.
GLuint Alaska::CreateCubemap(void)
{
//	TGA_FILE image[6];
	GLuint texture;
...
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	//							cinfo.output_height*cinfo.output_components);
		...
		TheCube[i].wall=(GLubyte*)malloc(3*256*256*sizeof(GLubyte));
		//Load the image.
//		LoadTargaFile(SkyboxTexturesNames[i], &image[i]);
	//	read_JPEG_file( local_byteptr,CubeMapNames[i]);
		TheCube[i].wall=read_JPEG_file( CubeMapNames[i]);
		//Store that section.
		glTexImage2D(cube_map_directions[i], 0, GL_RGB, row_width/3, col_height,
					0, GL_RGB, GL_UNSIGNED_BYTE, TheCube[i].wall);
	}
...
///////////////////// function to read cube map /////////////////////
//This function creates a cube map, from global data.
GLuint Alaska::CreateCubemapa(void)
{
//	TGA_FILE image[6];
	GLuint texture;
	AUX_RGBImageRec *image;
...
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	//	read_JPEG_file( local_byteptr,CubeMapNames[i]);
	//	TheCube[i].wall=read_JPEG_file( CubeMapNames[i]);
		image = auxDIBImageLoad( CubeMapNames[i]);
		//Store that section.
	//	glTexImage2D(cube_map_directions[i], 0, GL_RGB, row_width/3, col_height,
	//				0, GL_RGB, GL_UNSIGNED_BYTE, TheCube[i].wall);
		glTexImage2D(cube_map_directions[i], 0, GL_RGB,
			image->sizeX, image->sizeY, 0, GL_RGB, GL_UNSIGNED_BYTE, image->data);
	//	image->sizeX, image->sizeY, 0, GL_RGB, GL_UNSIGNED_BYTE, image->data);
	}
...
///////////////////// function to read cube map /////////////////////
//This function creates a cube map, from global data.
GLuint Alaska::CreateBottomMap(void)
{
//	TGA_FILE image[6];
	GLuint texture;
...
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	//							cinfo.output_height*cinfo.output_components);
		...
		TheBottom[i].wall=(GLubyte*)malloc(3*256*256*sizeof(GLubyte));
		//Load the image.
//		LoadTargaFile(SkyboxTexturesNames[i], &image[i]);
	//	read_JPEG_file( local_byteptr,CubeMapNames[i]);
		TheBottom[i].wall=read_JPEG_file( "textures/bottom.jpg");
		//Store that section.
		glTexImage2D(cube_map_directions[i], 0, GL_RGB, row_width/3, col_height,
					0, GL_RGB, GL_UNSIGNED_BYTE, TheBottom[i].wall);
	}
...
GLuint Alaska::init_normals_texturemap()
{
	GLuint texture;
	txpnt = (GLubyte *) malloc(64*3*64);
	if (txpnt == NULL)
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 64, 64,
				0, GL_RGB, GL_UNSIGNED_BYTE, txpnt);
	return texture;
}
...
//将纹理缩放到[0-255]之间
	for (int i=0;i<64;i++)
	{
		...
		{
			*(txpnt+3*i*64+j)   = (GLubyte)(normals[i][j/3][0]*wind*127. +127.);
			*(txpnt+3*i*64+j+1) = (GLubyte)(normals[i][j/3][1]*wind*127. +127.);
			*(txpnt+3*i*64+j+2) = (GLubyte)(normals[i][j/3][2]*wind*127. +127.);
		}
	}
...
void Alaska::draw_the_land()
{
	...
	glTranslatef(-75,50,-70);//left-right, depth , up_down
...
//	glRotatef(anglex,0,1,0);//rock the boat
	...
	glTranslated (-127.0, -255.0, -10.0);//(-=left,-=forward,-=down) when -90 above
	mytranslate(watertoworld,-127.0,-255.0,-10.0);
...
//这两个参数负责将整个海平面块分为四个部分画出来,由于存在场景的旋转,实际上是在调整X,Z
	...
	if (cgswitch==-1)unset_cube_cg();
	glPopMatrix();
	glMatrixMode(GL_MODELVIEW);		// this line returns mode to modelview
...
void	Alaska::pre_choppy()
{
	...
	dir=-1;
	if (FFT2D(mDeltaX, NX, NY, dir)==FALSE){cout<<"false on DXFFT"<<endl;exit( 0);}
	dir=-1;
	if (FFT2D(mDeltaY, NX, NY, dir)==FALSE){cout<<"false on DYFFT"<<endl;exit( 0);}
