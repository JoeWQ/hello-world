...
function ScrollViewExpand:callListener_(event)
    ...
    elseif event.name == self.EVENT_SCROLLEND then
        --延迟一帧让设置成让自己非运动
        self:delayCall(c_func(self.setTouchState, self,self.EVENT_SCROLLEND), 0.0001)
    end
        ...
        for i=startIndex,length do
            ...
            if not isCache then
                ...
                if groupData.updateFunc then
                    groupData.updateFunc(groupData.data[i],view)
                end
            end
            createNums = i
...
function ScrollViewExpand:initCellGroupProp( groupData,groupIndex )
    ...
    for i,v in ipairs(groupData.data) do
        ...
        -- echo(index,"__________",totalViewLength,groupIndex,x,y)
        --拿的是哪个view
    ...
    {
        {
            ...
            grouBg = 1
            updateFunc                       --刷新item的函数 updateFunc(data,view),如果有这个刷新函数,每当从缓存里面拿到的view都会做一次刷新,
            ...
            updateCellFunc            --更新cell的方法,如果有这个方法,表示是item重复使用的, u
                                      --pdateCellFunc(data,view),表示给与这个view重新赋值数据
            ...
            if info then
                ...
                if view then
                    ...
                    if view.__cellData ~= cellData or (isInit == 1) then
                        ...
                        groupData.updateCellFunc(cellData,view)
                        --那么调整view的位置 
