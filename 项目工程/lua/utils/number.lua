...
function number.numBorder( value,area1,area2 )
	...
	if number.isNan(value) then
		print(value, debug.traceback("__传入的数据是空的"))
		return area1
	end
...
function number.fract(floatValue)
    local MATH_EPS = 0.000002
    --在一个整数的左侧的非常临近的区域
    local _intValue = math.floor(floatValue +MATH_EPS )
    if _intValue == math.ceil(floatValue)then    
        return 0           
    end
    --在某个整数的右侧的非常临近的区域
    _intValue = math.ceil(floatValue - MATH_EPS)
    local _floorValue = math.floor(floatValue)
    if _intValue ==_floorValue then
        return 0
    end
    return math.abs(floatValue - _floorValue)
end
--舍入
