...
function ServerOther:init()
	...
	if not self:checkHasMethod(MethodCode.user_state_315) then
		local connInfo = self:turnRequestSave( {clientInfo=clientInfo, cid = LoginControler:getCid(),nodeServer = ServiceData.nodeServerName}, MethodCode.user_state_315,c_func(self.onConnectBack, self ))
		--把315请求放到最后面去
	...
	if not self:checkHasMethod(MethodCode.sys_init) then
		connInfo = self:turnRequestSave( {ver = AppInformation:getVersion()}, MethodCode.sys_init,nil,true,false,false)
		table.insert(self.connCache, 1,{connInfo} )
	end
	-- self._isClose = false
...
--判断是否拥有哪个请求
function ServerOther:checkHasMethod( method )
	if self.curConn then
...
--连接完成后的回调
function ServerOther:onConnectBack( result )
	self._isClose = false
	self._isConnect = true
	-- dump(result,"________initBack")
	if self.socket.initComplete then
		dump(result,"___connectback")
		self.socket:initComplete(result.result.aesKey,result.result.rid,tonumber(result.result.initRequestId) )
	end
	
	self.id = (tonumber(result.result.initRequestId) +1 ) or self.id                
...
-- 更新baseData
function ServerOther:updateBaseData(baseData)
	--先把 非userdata的数据拆开
...
function ServerOther:showErrorPopup(errorInfo)
	local errorCode = tonumber(errorInfo.code)
	if not table.find(ServerOther.SPECIAL_ERRORCODE, errorCode) then
		return
	end
...
function ServerOther:sendRequest(params, methodid, callBack, oneway, outLoading, needErrorCall)
	...
	local tempFunc = function ()
		...
		if self.curConn and #self.curConn > 0 then
			...
			if self._isClose and methodid ~= MethodCode.sys_init  and methodid ~=  MethodCode.user_state_315 then
...
-- 重新发送当前请求
function ServerOther:reSendRequest()
...
-- 取消当前请求
function ServerOther:cancelRequest()
...
--确定发送
function ServerOther:sureSend( info )
	...
	--需要经过一道数据转化
	self.socket:sendRequest( self:turnSendRequest(info)   )
...
--输出一个responcestring
function ServerOther:toResponceString( responce )
	local str =""
...
--手动关闭
function ServerOther:handleClose()
	if self.socket then
		self._isConnect = false
		self._isClose = true
		self.socket:close()
	...
	if DEBUG_CONNLOGS > 0 then
		...
		if device.platform == "windows" or device.platform =="mac" then
			...
			if targetFile == nil then
				echo(targetFileName)
				echo(errorMsg, '========================================')
				echoWarn("ServerOther:saveLogs targetFile is nil")
				return
			end
