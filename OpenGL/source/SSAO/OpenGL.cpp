...
struct       UserData
{
	...
	SSAORender          *_ssaoRender;
	NormalBlur           *_normalBlur;
...
void        Init(GLContext    *_context)
{
	...
	_user->_ssaoRender = new   SSAORender();
	_user->_normalBlur = new   NormalBlur();
	_user->_renderSphere->initWithFile("tga/Earth512x256.tga");
	_user->_afterProgram = GLProgram::createWithFile("shader/ssao/ssao_light.vsh", "shader/ssao/ssao_light.fsh");
...
//////////////////////////////////////程序数据/////////////////////////////////////
 ...
 //光照的颜色
	...
	_user->_ambientColor = GLVector3(0.5f,0.5f,0.5f);
...
//位置
	...
	_user->_ssaoMapLoc = _user->_afterProgram->getUniformLocation("u_ssaoMap");
...
//
	...
	_context->setNearFarPlane(GLVector2(0.0f,10.0f));
...
//	glClear(GL_COLOR_BUFFER_BIT| GL_DEPTH_BUFFER_BIT);
	...
	_user->_renderSphere->setPosition(GLVector3(0.0f,0.0f,-3.2f));
	_user->_renderSphere->draw(_user->_projectMatrix, 0x1);

	_user->_renderSphere->setPosition(GLVector3(-0.5f,0.5f,-4.0f));
	_user->_renderSphere->draw(_user->_projectMatrix,0x1);
	...
	int        _ssaoMap = _user->_ssaoRender->loadSSAOTexture(_user->_projectMatrix, _user->_globleTextureId[0], _user->_globleTextureId[1]);
	glBindFramebuffer(GL_FRAMEBUFFER, originFramebufferId);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
//执行后处理着色器
	_user->_afterProgram->enableObject();
	glBindBuffer(GL_ARRAY_BUFFER, _context->loadBufferIdentity());
...
//三重纹理
	...
	glUniform1i(_user->_ssaoMapLoc,4);
//与光照有关的数据
...
void         ShutDown(GLContext  *_context)
{
	...
	_user->_ssaoRender->release();
	_user->_normalBlur->release();
