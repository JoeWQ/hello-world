...
bool    CameraShader::loadShaderSource(const char *vsFile, const char *fsFile)
{
	...
	_cropMatrixLoc = _renderProgram->getUniformLocation("g_CropMatrix");
	_shadowMapArrayLoc = _renderProgram->getUniformLocation("g_SegmentShadow");
	_baseMapLoc = _renderProgram->getUniformLocation("g_BaseMap");
//	_lightVPSBMatrixLoc = _renderProgram->getUniformLocation("u_lightVPSBMatrix");
	_normalSegmentsLoc = _renderProgram->getUniformLocation("g_NormalSegments");
	_lightDirectionLoc = _renderProgram->getUniformLocation("g_LightDirection");
	_eyePositionLoc = _renderProgram->getUniformLocation("g_EyePosition");
	return true;
}
...
void  CameraShader::setCropMatrix(const glk::Matrix cropMatrix[4],const int size)
{
#ifdef __DEBUG_CAMERA_SHADER_
	int    _defaultProgram;
	glGetIntegerv(GL_CURRENT_PROGRAM, &_defaultProgram);
	...
	if (_cropMatrixLoc>=0)
	{
		memcpy(_cropMatrix, cropMatrix, size * sizeof(glk::Matrix));
		glUniformMatrix4fv(_cropMatrixLoc, 4, GL_FALSE, cropMatrix->pointer());
	}
}
