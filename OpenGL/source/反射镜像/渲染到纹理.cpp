...
//time
	...
	GLProgram      *otherObject;
	GLuint                otherBaseMapLoc;
};
...
//
	...
	//
		...
		_user->otherObject = GLProgram::createWithFile("shader/tex_normal.vsh", "shader/tex_normal.fsh");
		_user->otherBaseMapLoc = _user->otherObject->getUniformLocation("u_baseMap");
//create framebuffer
...
//resume
	...
	_user->otherObject->enableObject();
	glBindBuffer(GL_ARRAY_BUFFER, _user->vertexBufferId);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, _user->renderTextureId);
	glUniform1i(_user->otherBaseMapLoc, 0);
