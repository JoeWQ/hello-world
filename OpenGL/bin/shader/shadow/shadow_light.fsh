...
void    main()
{
	...
	float     _diffuse = max(0.0,dot(u_lightDirection,_normal));

	vec3      _reflect = reflect(-u_lightDirection,_normal);
	vec3      _eyePosition = normalize(u_eyePosition - v_position);
	float     _specular = max(0.0,dot(_reflect,_eyePosition));

    float     _shadow_factor = lightShadowfactor(_normal);
