...
//	vec3      reflect_dir = normalize(reflect(-light_dir, light_normal));
    ...
    vec3      halfwayVector = normalize(eye_dir + light_normal);
//	float     specular=max(0.0, dot(eye_dir , reflect_dir));
    float     specular = max(0.0,dot(halfwayVector,light_normal));

	outColor.xyz=baseColor.xyz*u_ambientColor + baseColor.xyz*u_lightColor*diffuse +u_lightColor*pow(specular,64.0)*u_lightPower;
