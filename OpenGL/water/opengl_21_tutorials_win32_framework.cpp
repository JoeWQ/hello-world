...
bool COpenGLRenderer::Init()
{
	...
	Error |= !Scene.Load("scene.vs", "scene.fs");
	Error |= !Water.Load("water.vs", "water.fs");
	...
	glUseProgram(Water);
	glUniform1i(glGetUniformLocation(Water, "ReflectionTexture"), 0);
	glUniform1i(glGetUniformLocation(Water, "RefractionTexture"), 1);
	glUniform1f(glGetUniformLocation(Water, "WMSDWMR"), WMS / (float)WMR);
	glUniform1f(glGetUniformLocation(Water, "WMSDWMRM2"), WMS / (float)WMR * 2.0f);
	glUseProgram(0);

	Scene.UniformLocations = new GLuint[5];
	Scene.UniformLocations[0] = glGetUniformLocation(Scene, "ClipType");
	Scene.UniformLocations[1] = glGetUniformLocation(Scene, "WaterLevel");
	Scene.UniformLocations[2] = glGetUniformLocation(Scene, "Texturing");
	Scene.UniformLocations[3] = glGetUniformLocation(Scene, "NormalMatrix");
	Scene.UniformLocations[4] = glGetUniformLocation(Scene, "ModelMatrix");

	Water.UniformLocations = new GLuint[4 + MAX_WAVES * 5];
	Water.UniformLocations[0] = glGetUniformLocation(Water, "Time");
	Water.UniformLocations[1] = glGetUniformLocation(Water, "WaterLevel");
	Water.UniformLocations[2] = glGetUniformLocation(Water, "CameraPosition");
	Water.UniformLocations[3] = glGetUniformLocation(Water, "NormalMatrix");
	CString Variable;
	for(int Wave = 0; Wave < MAX_WAVES; Wave++)
	{
		Variable.Set("Waves[%d].Position", Wave);
		Water.UniformLocations[4 + Wave * 5 + 0] = glGetUniformLocation(Water, Variable);
		Variable.Set("Waves[%d].StartTime", Wave);
		Water.UniformLocations[4 + Wave * 5 + 1] = glGetUniformLocation(Water, Variable);
		Variable.Set("Waves[%d].Speed", Wave);
		Water.UniformLocations[4 + Wave * 5 + 2] = glGetUniformLocation(Water, Variable);
		Variable.Set("Waves[%d].MaxY", Wave);
		Water.UniformLocations[4 + Wave * 5 + 3] = glGetUniformLocation(Water, Variable);
		Variable.Set("Waves[%d].FrequencyMPIM2", Wave);
		Water.UniformLocations[4 + Wave * 5 + 4] = glGetUniformLocation(Water, Variable);
	}
...
void COpenGLRenderer::Render(float FrameTime)
{
	...
	glActiveTexture(GL_TEXTURE1); glBindTexture(GL_TEXTURE_2D, RefractionTexture);
	glEnableClientState(GL_VERTEX_ARRAY);
	glBindBuffer(GL_ARRAY_BUFFER, WaterVerticesVBO);
	glVertexPointer(2, GL_FLOAT, 8, nullptr);
	glUseProgram(Water);
	glUniform1f(Water.UniformLocations[0], (float)GetTickCount() * 0.001f);
	glUniform1f(Water.UniformLocations[1], WaterLevel);
	glUniform3fv(Water.UniformLocations[2], 1, &Camera.Position);
	vec3 X = Camera.X, Y = vec3(0.0f, 1.0f, 0.0f), Z = cross(X, Y);
	NormalMatrix = mat3x3(X.x, Y.x, Z.x, X.y, Y.y, Z.y, X.z, Y.z, Z.z);
	glUniformMatrix3fv(Water.UniformLocations[3], 1, GL_FALSE, &NormalMatrix);
...
void COpenGLRenderer::RenderScene(int ClipType)
{
	...
	glUseProgram(Scene);
	glUniform1i(Scene.UniformLocations[0], ClipType);
	glUniform1f(Scene.UniformLocations[1], WaterLevel);

	glUniform1i(Scene.UniformLocations[2], 1);//Texturing最后输出的颜色是否需要经过采样

	glUniformMatrix3fv(Scene.UniformLocations[3], 1, GL_FALSE, &mat3x3());
	glUniformMatrix4fv(Scene.UniformLocations[4], 1, GL_FALSE, &mat4x4());
	...
	*/
	...
	glUniform1i(Scene.UniformLocations[2], 0);

	glDrawArrays(GL_QUADS, 180, 4);

	glUniformMatrix3fv(Scene.UniformLocations[3], 1, GL_FALSE, &mat3x3(ModelMatrix));
	glUniformMatrix4fv(Scene.UniformLocations[4], 1, GL_FALSE, &ModelMatrix);
...
{
	float s = 1.0f * x / (float)(Width - 1);
	float t = 1.0f - (float)y / (Height - 1);
	...
	float D = -dot(Normal, vec3(0.0f, WaterLevel, 0.0f));//空间平面方程的最后一个参数

	float NdotR = -dot(Normal, Ray);//法线与射线的点成,求出的是夹角的余弦值

	if(NdotR != 0.0f)//如果射线与法线不是垂直关系
	{
		//NDotR代表着摄像机与目标点的连线的反向向量与水平面所成的夹角的正弦值
		//因此下面求出的是斜边的长度
		float Distance = (dot(Normal, Camera.Position) + D) / NdotR;//求出摄像机与目标点的连线与水平面之间的交点的距离

		if(Distance > 0.0)
		{
			vec3 Position = Camera.Position + Ray * Distance;

			const float WMSD2 = WMS / 2.0f; 
			const float MWMSD2 = -WMS / 2.0f;
...
void COpenGLRenderer::InitWaterVertexBuffer()
{
	...
	const float WMSD2 = WMS / 2.0f, MWMSD2 = -WMSD2, WMSDWMR = WMS / (float)WMR;
...
{
	char *AppName = "Interactive water surface, reflection and refraction";
